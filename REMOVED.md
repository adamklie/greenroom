...
| Triage | `backend/app/routers/triage.py`, `backend/app/schemas/triage.py`, `backend/app/models/triage.py`, `frontend/src/pages/Triage.tsx`, `api.triage.*` + `TriageItem` interface in client.ts, `triage_pending` from `DashboardStats` (schema + frontend), `TriageItem` creation in `backend/app/services/bootstrap.py` | `triage_queue` (drop via `scripts/drop_simplified_tables.py`) | The Triage queue was a holding pen between "scan finds a file" and "user assigns it to a song." With the Library UI letting users assign songs directly on the AudioFile row, the queue was redundant. Bootstrap now just logs unlinked files to stdout. | Low | 361898f (callers), 45c4afb (router + page), Phase 11.3 commit (model + REMOVED.md) |
| Sync | `backend/app/routers/sync.py`, `frontend/src/pages/Sync.tsx`, `api.sync.*` in client.ts | none | The Sync page only triggered `make after-practice` / `make weekly` via the browser. The user runs those directly from CLI now (and eventually via an `/ingest-recordings` skill). Removing the in-app trigger drops two HTTP endpoints, a page, and an entire React-Query dependency for what's already a one-line shell command. | Low | Phase 12 commit |
